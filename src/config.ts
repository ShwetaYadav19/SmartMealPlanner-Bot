...
// Razorpay vars are only required when payment is enabled
const PAYMENT_VARS = [
  'RAZORPAY_KEY_ID',
  'RAZORPAY_KEY_SECRET',
  'RAZORPAY_PLAN_ID',
...
export function loadConfig(): Config {
  ...
  // Only require Razorpay vars when payment is enabled
  const requirePayment = process.env.REQUIRE_PAYMENT === 'true';
  if (requirePayment) {
    const missingPayment = PAYMENT_VARS.filter((v) => !process.env[v]);
    missing.push(...missingPayment);
  }
